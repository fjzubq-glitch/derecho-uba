...
export interface ClaseClientProps {
  initialData: {
    materia: MateriaData;
    clase: Clase | null;
    adjacentes: { numero: number; titulo: string }[];
    grants: string[];
  };
}

export default function ClaseClient({ initialData }: ClaseClientProps) {
  const params = useParams();
  const router = useRouter();
  const materiaSlug = params.materia as string;
  const numero = params.numero as string;

  const [materia, setMateria] = useState<MateriaData | null>(initialData.materia);
  ...
  const [grants, setGrants] = useState<string[]>(initialData.grants);
        ...
        <div className="pad-lateral" style={{ padding: "60px 48px 120px" }}>
          ...
          {clase.archivos.length === 0 ? (
            ...
            <>
              ...
              {(() => {
                const puedeVerTodo = esAdmin || tieneAcceso;
                const privadosVisibles = puedeVerTodo
                  ? clase.archivos.filter((a) => TIPOS_PRIVADOS.includes(a.tipo))
                  : clase.archivos.filter((a) => TIPOS_PRIVADOS.includes(a.tipo) && grants.includes(a.id));
                  ...
                  <>
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          {privadosVisibles.length} archivo{privadosVisibles.length !== 1 ? "s" : ""}
                        </span>
                      </div>
                    </div>
                    )}

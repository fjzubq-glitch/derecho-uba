...
async function getDashboardPayload(dias: number | null) {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Las consultas son costosas (hasta 200k filas). Cacheamos el payload
    // 120s para no golpear Supabase en cada visita al panel.
    const payload = await unstable_cache(
      () => getDashboardPayload(dias),
      ["admin-dashboard", String(dias)],
      { revalidate: 120 },
    )();
